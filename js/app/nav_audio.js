Music = new Object({
		...
		GoAll : function (filter, init, share){
			var params;			
			if(init==1){
				//если не в режиме share то обнул€ем объекты
				...
				function( data ) {
					if(data.auth_status=="success" ){
						if(data.method_status=="success"){
							...
							//если загрузка инициализирующа€
							if(params.initial==1){
								//дополн€ем список песен
		...
		Display: function(resp_obj, params){ 
			...
			if(params.initial == 1){
				...
				if(params.user_id!=null){
					...
					if(params.user_id == User.I.id){
						$("#page_music .topheader h1").html("ћо€ музыка");
						//если моих песен нет, то пишем подсказку
						if(resp_obj.total_all==0)
							$("#page_music_list").html("<div class='no-data-info'><p>«десь будут отображатьс€ песни, которые вы добавите в свой список</p><p>¬се доступные песни можно найти на √лавной станице в разделе ћузыка</p></div>");
					}
					else{
					...
					$("#page_music .topheader h1").html("ћузыка");
				}
			...
			$.each(resp_obj.musics, function(key,obj){
				...
				//добавл€ем кнопки снизу
				html_menu="";
				html_menu+='<div class="nation">'+Environment.Nations.modify(Environment.Nations.nation_by_id(obj.lang).title, "а€")+'</div>';
				html_menu+="<div class='options'>";				
				html_menu+='<a base="music" t_key="'+obj.num+'" mark="plus" class="ui-btn ui-btn-inline ui-mini ui-corner-all ui-icon-thumbs-o-up ui-btn-icon-notext btn-like" >Ќравитс€</a>';
				html_menu+='<a base="music" t_key="'+obj.num+'" mark="minus"  class="ui-btn ui-btn-inline ui-mini ui-corner-all ui-icon-thumbs-o-down ui-btn-icon-notext btn-unlike" >Ќе нравитс€</a>';
		...
		Utils : {
			get_obj_index: function(objs, num){
				...
				$.each(objs, function(key, obj){
					//если вход€щее сообщение из существующего списка то обновл€ем счетчик
					if(obj.num==num){
						found_i=i;
					}
					i++;
			...
			display_found_number : function(total){
				if(total>0){
					$("#page_music .found_number").html(NumSklon(total, ['Ќайдена', 'Ќайдены', 'Ќайдено'])+" "+total+" "+NumSklon(total, ['песн€', 'песни', 'песен']));
				}
				else
					$("#page_music .found_number").html("Ќе найдено ни одной песни");
			}
		},
		link : {
			refresh_nation_footer : function(){
				
				$("#page_music_nation_popup").html("<option value=0>¬се народы</option>"+Environment.Nations.nations_selectmenu());
				$("#page_music_nation_popup").change (Music.link.btn_nations_onClick);
			},
			refresh_navbar : function(){
				if(Exch.to.type!=null){ //если в режиме share, то скрываем нижний навбар и показываем меню прин€ть-отклонить
			...
			refresh_like_menu : function(obj){
				var vote_html="";
				var page_id = $(":mobile-pagecontainer" ).pagecontainer( "getActivePage" ).attr("id");
				//если мы на странице все музыки, то обновл€ем нижнее меню с рейтингом и комментари€ми
				...
				if(Exch.to.type!=null){
					...
					if(obj.added>0){
						...
						// круглый плюс $("#mus_"+ obj.num+" .btn-add").html('<a href="#" class="ui-btn ui-icon-plus ui-btn-icon-notext ui-corner-all ui-mini">ƒобавить</a>');
